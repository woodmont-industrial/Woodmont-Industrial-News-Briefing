 ...
 * REJournals Scraper
 *
 * Scrapes REJournals industrial/CRE news.
 * RSS feed blocked by Cloudflare, so we scrape directly.
 */
...
export class REJournalsScraper extends BaseScraper {
    ...
    async extractArticles(target: ScraperTarget, page: Page | null, html?: string): Promise<NormalizedItem[]> {
        ...
        try {
            ...
            // Handle cookie/Cloudflare if needed
            try {
                const acceptBtn = await page.$('button:has-text("Accept"), button:has-text("I Agree")');
                if (acceptBtn) {
            ...
            const items = await page.evaluate(() => {
                ...
                allLinks.forEach(el => {
                    ...
                    if (!link.includes('rejournals.com')) return;
                    ...
                    // REJournals article patterns
                    const isArticleLink = (
                        link.match(/rejournals\.com\/\d{4}\/\d{2}\//) || // Date-based URLs
                        (link.includes('rejournals.com/') &&
                         !link.endsWith('.com/') &&
                         !link.endsWith('/industrial/') &&
                         !link.endsWith('/industrial') &&
                         link.split('/').filter(Boolean).length > 3)
                    );
            ...
            console.error(`[REJournals] Extraction error:`, (error as Error).message);
        }
