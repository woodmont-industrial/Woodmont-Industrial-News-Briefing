/**
 * SFBJ (South Florida Business Journal) Scraper
 *
 * Scrapes South Florida Business Journal commercial real estate news.
...
export class SFBJScraper extends BaseScraper {
    ...
    private async extractFromPage(page: Page, target: ScraperTarget): Promise<NormalizedItem[]> {
        ...
        try {
            ...
            await page.goto('https://www.google.com/search?q=site:bizjournals.com/southflorida+commercial+real+estate', {
            ...
            const items = await page.evaluate(() => {
                ...
                allLinks.forEach(el => {
                    ...
                    if (!link.includes('bizjournals.com/southflorida')) return;
                    ...
                    if (link.includes('/category/') || link.includes('/tag/') ||
                        ...
                        link.includes('/account/') || link.includes('/apps/') ||
                        link.includes('/bizwomen/') || link.includes('/event/')) return;

                    // SFBJ article pattern: /southflorida/news/YYYY/MM/DD/{slug}.html
                    const articlePattern = /\/southflorida\/news\/\d{4}\/\d{2}\/\d{2}\/[^/]+\.html/;
                    if (!articlePattern.test(link)) return;
            ...
            console.log(`[SFBJ] Extracted ${articles.length} articles from ${target.label}`);
        } catch (error) {
            console.error(`[SFBJ] Extraction error:`, (error as Error).message);
        }
    ...
    private extractFromHTML(html: string): NormalizedItem[] {
        const articles: NormalizedItem[] = [];
        const linkPattern = /href="(https?:\/\/(?:www\.)?bizjournals\.com\/southflorida\/news\/\d{4}\/\d{2}\/\d{2}\/[^"]+\.html)"/g;
        const seen = new Set<string>();
